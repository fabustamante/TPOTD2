...
/**
	...
	@brief Inicializa los bloques gc
 	...
 	@return void
 */
void gc_init(parser_block_t* gc_block,uint8_t* axis_0,uint8_t* axis_1,uint8_t* axis_linear)
{
//	int i;
...
void vGCodeDispatchTask (void* Parameters)
{
	...
	gc_init(&gc_block,&axis_0,&axis_1,&axis_linear);
...
//			gcode_parser(token);
		...
		xQueueSendToBack( xQueue_GCodeBlock, &gc_block, portMAX_DELAY );
	}
}
